...
namespace OPS.Obfuscator.Editor.Project.PostAssemblyBuild.Pipeline.Component.Member.Field
{
    public class FieldObfuscationComponent : AMemberObfuscationComponent<FieldReference, FieldKey>
    {
        ...
        public const String CObfuscate_Field_Struct = "Obfuscate_Field_Struct";
        ...
        public override ObfuscatorContainer GetGuiContainer(ComponentSettings _ComponentSettings)
        {
            ...
            var_ObfuscateSerializeableRow.Notification_Info = "Enable this setting to obfuscate fields that are marked with the [SerializeField] attribute.";
            ...
            var_ObfuscateUnityPublicFieldRow.Notification_Info = "Enable this setting to obfuscate public fields in monobehaviour/serializeable/networkbehaviour subclasses.";
            ...
            // Obfuscate Enum Fields
            Row_Boolean var_EnumFieldRow = new Row_Boolean("Obfuscate Enum Values:", _ComponentSettings, CObfuscate_Field_Enums);
            var_EnumFieldRow.Notification_Info = "Enable this setting to obfuscate enum fields. Note that since enums are considered serializable, their members (including fields) are also treated as serializable. However, you can still obfuscate enum field names by activating this option, regardless of whether serializable field obfuscation is disabled.";
            var_EnumFieldRow.Notification_Warning = "Note that if you choose to obfuscate enums and then serialize them (e.g., to save Unity or game data), it's essential to maintain consistent naming between builds to ensure compatibility and prevent data corruption.";
            var_Content.AddRow(var_EnumFieldRow);

            // Obfuscate Struct Fields
            Row_Boolean var_StructFieldRow = new Row_Boolean("Obfuscate Struct Fields:", _ComponentSettings, CObfuscate_Field_Struct);
            var_StructFieldRow.Notification_Info = "Enable this setting to obfuscate struct fields. Note that since structs are considered serializable, their members (including fields) are also treated as serializable. However, you can still obfuscate struct field names by activating this option, regardless of whether serializable field obfuscation is disabled.";
            var_StructFieldRow.Notification_Warning = "Note that if you choose to obfuscate structs and then serialize them (e.g., to save Unity or game data), it's essential to maintain consistent naming between builds to ensure compatibility and prevent data corruption.";
            var_Content.AddRow(var_StructFieldRow);
        ...
        private void OnAnalyse_A_FindFieldDefinitionShouldBeNotRenamed(AssemblyInfo _AssemblyInfo)
        {
            ...
            foreach (TypeDefinition var_TypeDefinition in _AssemblyInfo.GetAllTypeDefinitions())
            {
                foreach (FieldDefinition var_FieldDefinition in var_TypeDefinition.Fields)
                {
                    ...
                    // 3. Check the field accessibility settings.
                    ...
                    {
                        // Cause
                        String var_DoNotObuscateCause = "Because of your defined field accessibility obfuscation settings.";

                        // Add do not rename.
                        this.OnAnalyse_Helper_SkipField(var_FieldDefinition, var_DoNotObuscateCause);
                    }

                    // 4. Check the field serializable settings.
                    if (TypeDefinitionHelper.IsTypeOrBaseSerializable(var_FieldDefinition.DeclaringType))
                    {
                        // The declaring type is serializable or some base is serializeable.

                        // Check if the obfuscation of serializeable types or the obfuscation of serializeable fields is deactivated.
                        if (!this.Step.Settings.Get_ComponentSettings_As_Bool(PostAssemblyBuild.Pipeline.Component.Member.Type.TypeObfuscationComponent.CSettingsKey, PostAssemblyBuild.Pipeline.Component.Member.Type.TypeObfuscationComponent.CObfuscate_Class_Serializable)
                            || !this.Step.Settings.Get_ComponentSettings_As_Bool(this.SettingsKey, CObfuscate_Field_Serializable))
                        {
                            // Is serializable and the obfuscation of serializeable types is deactivated.

                            if (var_FieldDefinition.DeclaringType.IsEnum || var_FieldDefinition.DeclaringType.IsValueType)
                            {
                                // The type is an enum or a value type (struct), they have their own settings.
                            }
                            else if(var_FieldDefinition.IsPublic)
                            {
                                // Cause
                                String var_DoNotObuscateCause = "Because of the deactivated 'Obfuscate serializable classes' setting.";
                        ...
                        // Check if the declaring type is an enum or a struct, they have their own settings.
                        if (var_FieldDefinition.DeclaringType.IsEnum)
                        {
                            ...
                            {
                                // Cause
                                String var_DoNotObuscateCause = "Because of the deactivated 'Obfuscate enum values' setting.";

                                // Add do not rename.
                                this.OnAnalyse_Helper_SkipField(var_FieldDefinition, var_DoNotObuscateCause);
                            }
                        }
                        else if (var_FieldDefinition.DeclaringType.IsValueType)
                        {
                            // Is struct field

                            // Check Struct Setting
                            if (!this.Step.Settings.Get_ComponentSettings_As_Bool(this.SettingsKey, CObfuscate_Field_Struct))
                            {
                                // Cause
                                String var_DoNotObuscateCause = "Because of the deactivated 'Obfuscate struct fields' setting.";
