...
namespace OPS.Obfuscator.Editor.Project.PostAssemblyBuild.Pipeline.Component.Member.Type
{
    public class TypeObfuscationComponent : AMemberObfuscationComponent<TypeReference, TypeKey>
    {
        ...
        public const String CObfuscate_Class_Struct = "Obfuscate_Class_Struct";
        public const String CObfuscate_Class_Enum = "Obfuscate_Class_Enum";
        ...
        public override ObfuscatorContainer GetGuiContainer(ComponentSettings _ComponentSettings)
        {
            ...
            // Subclasses
            Row_Section var_SubclassesRow = new Row_Section("Advanced:");
            ...
            var_ObfuscateAbstractClassRow.Notification_Info = "Enable this setting to obfuscate abstract classes.";
            ...
            var_ObfuscateGenericClassRow.Notification_Info = "Enable this setting to obfuscate generic classes.";
            ...
            var_ObfuscateSerializableClassRow.Notification_Info = "Enable this setting to obfuscate serializeable classes or classes that are marked with the [Serializeable] attribute.";
            ...
            // Obfuscate Struct Classes
            Row_Boolean var_ObfuscateStructClassRow = new Row_Boolean("Obfuscate Structs:", _ComponentSettings, CObfuscate_Class_Struct);
            var_ObfuscateStructClassRow.Notification_Info = "Enable this setting to obfuscate structs. Although structs are treated as serializable, you can still obfuscate their names by activating this option, even if serializable obfuscation is disabled.";
            var_ObfuscateStructClassRow.Notification_Warning = "Note that if you choose to obfuscate structs and then serialize them (e.g., to save Unity or game data), it's essential to maintain consistent naming between builds to ensure compatibility and prevent data corruption.";
            var_Content.AddRow(var_ObfuscateStructClassRow);

            // Obfuscate Enum Classes
            Row_Boolean var_ObfuscateEnumClassRow = new Row_Boolean("Obfuscate Enums:", _ComponentSettings, CObfuscate_Class_Enum);
            var_ObfuscateEnumClassRow.Notification_Info = "Enable this setting to obfuscate enums. Although enums are treated as serializable, you can still obfuscate their names by activating this option, even if serializable obfuscation is disabled.";
            var_ObfuscateEnumClassRow.Notification_Warning = "Note that if you choose to obfuscate enums and then serialize them (e.g., to save Unity or game data), it's essential to maintain consistent naming between builds to ensure compatibility and prevent data corruption.";
            var_Content.AddRow(var_ObfuscateEnumClassRow);

            // Unity Subclasses
            Row_Section var_UnitySublcassesRow = new Row_Section("Advanced - Unity Subclasses:");
            ...
            var_ObfuscateMonoBehaviourClassRow.Notification_Info = "Enable this setting to obfuscate MonoBehaviour subclasses. This requires the activation of the namespace obfuscation setting.";
            ...
            var_ObfuscateUnityClassExternalRow.Notification_Info = "Enable this setting to obfuscate MonoBehaviour subclasses in external assemblies you set in the assembly settings. This setting will obfuscate the classes in the 'Past 2018.2' way.";
            ...
            var_ObfuscateScriptableObjectClassRow.Notification_Info = "Enable this setting to obfuscate ScriptableObject subclasses. This requires the activation of the namespace obfuscation setting.";
            ...
            var_ObfuscatePlayAbleClassRow.Notification_Info = "Enable this setting to obfuscate Playable/PlayableAsset/PlayableBehaviour subclasses. This requires the activation of the namespace obfuscation setting.";
        ...
        {
            // Iterate all types and check if they should be renamed.
            foreach (TypeDefinition var_TypeDefinition in _AssemblyInfo.GetAllTypeDefinitions())
            {
                // 1. Check the compatibility components.
                ...
                // 3. Check the class accessibility settings.
                if (AccessibilityHelper.AccessibilityLevel_Type(var_TypeDefinition) == EAccessibilityLevel.Public && this.Step.Settings.Get_ComponentSettings_As_Bool(this.SettingsKey, CObfuscate_Class_Public)
                || AccessibilityHelper.AccessibilityLevel_Type(var_TypeDefinition) == EAccessibilityLevel.Protected && this.Step.Settings.Get_ComponentSettings_As_Bool(this.SettingsKey, CObfuscate_Class_Protected)
                || AccessibilityHelper.AccessibilityLevel_Type(var_TypeDefinition) == EAccessibilityLevel.Protected_And_Internal && this.Step.Settings.Get_ComponentSettings_As_Bool(this.SettingsKey, CObfuscate_Class_Protected) && this.Step.Settings.Get_ComponentSettings_As_Bool(this.SettingsKey, CObfuscate_Class_Internal)
                || AccessibilityHelper.AccessibilityLevel_Type(var_TypeDefinition) == EAccessibilityLevel.Protected_Or_Private && (this.Step.Settings.Get_ComponentSettings_As_Bool(this.SettingsKey, CObfuscate_Class_Protected) || this.Step.Settings.Get_ComponentSettings_As_Bool(this.SettingsKey, CObfuscate_Class_Private))
                || AccessibilityHelper.AccessibilityLevel_Type(var_TypeDefinition) == EAccessibilityLevel.Internal && this.Step.Settings.Get_ComponentSettings_As_Bool(this.SettingsKey, CObfuscate_Class_Internal)
                || AccessibilityHelper.AccessibilityLevel_Type(var_TypeDefinition) == EAccessibilityLevel.Private && this.Step.Settings.Get_ComponentSettings_As_Bool(this.SettingsKey, CObfuscate_Class_Private))
                { 
                }
                else
                {
                    // Cause
                    String var_DoNotObuscateCause = "Because of your defined class accessibility obfuscation settings.";
                ...
                // 4. Check the class serializable settings.
                if (TypeDefinitionHelper.IsTypeOrBaseSerializable(var_TypeDefinition))
                {
                    // Is serializable or some base is serializeable.

                    // Check if the obfuscation of serializeable types is deactivated.
                    if (!this.Step.Settings.Get_ComponentSettings_As_Bool(this.SettingsKey, CObfuscate_Class_Serializable))
                    {
                        // Is serializable and the obfuscation of serializeable types is deactivated.
                        
                        if (var_TypeDefinition.IsEnum || var_TypeDefinition.IsValueType)
                        {
                            // The type is an enum or a value type (struct), they have their own settings.
                        }
                        else
                        {
                            // Note: Serialization mostly requires the namespace to, so we skip the namespace at this point too.
                            String var_DoNotObuscateCause = "Because of the deactivation of the 'Obfuscate serializable classes' settings.";

                            this.DataContainer.RenameManager.AddDoNotObfuscate(Editor.Assembly.Mono.Member.EMemberType.Type, var_TypeDefinition, var_DoNotObuscateCause);
                            this.DataContainer.RenameManager.AddDoNotObfuscate(Editor.Assembly.Mono.Member.EMemberType.Namespace, var_TypeDefinition, var_DoNotObuscateCause);

                            Obfuscator.Report.Append(OPS.Editor.Report.EReportLevel.Info, "Skip Type [" + var_TypeDefinition.ToString() + "] " + var_DoNotObuscateCause);
                            Obfuscator.Report.Append(OPS.Editor.Report.EReportLevel.Info, "Skip Types Namespace [" + var_TypeDefinition.ToString() + "] " + var_DoNotObuscateCause);
                        }
                    }

                    // Check if the class is an enum or a struct, they have their own settings.
                    if (var_TypeDefinition.IsEnum)
                    {
                        // The type is an enum, check the enums own settings.
                        if (this.Step.Settings.Get_ComponentSettings_As_Bool(this.SettingsKey, CObfuscate_Class_Enum))
                        {
                            // Is serializable and the obfuscation of enums is activated.
                        }
                        else
                        {
                            // Is serializable and the obfuscation of enums is deactivated.

                            // Note: Serialization mostly requires the namespace to, so we skip the namespace at this point too.
                            String var_DoNotObuscateCause = "Because of the deactivation of the 'Obfuscate enum classes' settings.";
                    ...
                    else if (var_TypeDefinition.IsValueType)
                    {
                        // The type is a value type (struct), check the structs own settings.
                        if (this.Step.Settings.Get_ComponentSettings_As_Bool(this.SettingsKey, CObfuscate_Class_Struct))
                        {
                            // Is serializable and the obfuscation of structs is activated.
                        }
                        else
                        {
                            // Is serializable and the obfuscation of structs is deactivated.

                            // Note: Serialization mostly requires the namespace to, so we skip the namespace at this point too.
                            String var_DoNotObuscateCause = "Because of the deactivation of the 'Obfuscate struct classes' settings.";
